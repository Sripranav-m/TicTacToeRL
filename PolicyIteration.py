...
class TicTacToe:
    ...
    def policyIteration(self):
        # Loop over entire State Space
        for i in range(len(self.statespace)):
            # Start with a random policy for each state
            # Since all actions are not possible, find a random possible action
            board = self.decimalToBoard(self.statespace[i])
            zeroes_pos = self.getZeroesInBoard(board)
            if(len(zeroes_pos) > 0):
                action_idx = np.random.choice(range(len(zeroes_pos)), 1)[0]
                action = zeroes_pos[action_idx]
                self.policyIterationPolicy.append(np.array(action))

        # Value Function initially is taken as 0 for all states
        vOld = np.zeros(len(self.statespace))
        cnt = 1
        while(True):
            print("Policy Iteration: ",cnt)
            cnt += 1
            pPi = []
            # Average reward Matrix
            # Vpie = (I-gamma*pPi)inv * Avg Reward
            # First find the average reward matrix for all states
            avgRewardMat = np.zeros(len(self.statespace))
            for currState in range(0, len(self.statespace)):
                board = self.decimalToBoard(self.statespace[currState])
                action = self.policyIterationPolicy[currState]
                action_idx = self.naryToDecimal(np.array(action), self.boardDimension)
                pPi.append(self.PTF[currState][action_idx])
                avg_reward = 0
                for nxt_st in range(len(self.statespace)):
                    avg_reward += self.PTF[currState][action_idx][nxt_st] * self.rewardFunction[currState][action_idx][nxt_st]
                avgRewardMat[currState] = avg_reward
            pPi = np.array(pPi)
            eye = np.eye(len(self.statespace))
            mat = np.linalg.inv(np.subtract(eye, np.multiply(pPi, 0.1)))

            vPi = np.dot(mat, avgRewardMat)
            if(abs(LA.norm(vPi) - LA.norm(vOld)) < 0.01):
                break
            vOld = vPi

            #oldPolicy=copy.deepcopy(self.policyIterationPolicy)

            self.policyIterationPolicy.clear()
            
            # For each state in the statespace, now find the action that maximises
            # Rbar[x] + gamma*pPi*VPi
            # VPi is calculated above
            for currState in range(0, len(self.statespace)):
                board = self.decimalToBoard(self.statespace[currState])
                zeroes_pos = self.getZeroesInBoard(board)
                # max_action_idx = -1
                max_value = -101
                best_action = []
                for action in zeroes_pos:
                    # Put X
                    board[action[0]][action[1]] = 1
                    action_idx = self.naryToDecimal(np.array(action), self.boardDimension)
                    avg_reward = 0
                    for nxt_action in zeroes_pos:
                        if(nxt_action != action):
                            # Put O
                            board[nxt_action[0]][nxt_action[1]] = 2
                            decimal = self.boardToDecimal(board)
                            nxt_st = self.statespace.index(decimal)
                            avg_reward += self.PTF[currState][action_idx][nxt_st] * self.rewardFunction[currState][action_idx][nxt_st]
                            board[nxt_action[0]][nxt_action[1]] = 0
                    gamma_pv_term = np.dot(self.PTF[currState][action_idx], vPi)
                    tPi_vPi = avg_reward + 0.1*gamma_pv_term
                    if max_value < tPi_vPi:
                        max_value = tPi_vPi
                        best_action = action
                    board[action[0]][action[1]] = 0
                self.policyIterationPolicy.append(best_action)
            
            # isEqual=True
            # for i in range(len(self.statespace)):
            #     for j in range(2):
            #         if oldPolicy[i][j]!=self.policyIterationPolicy[i][j]:
            #             isEqual=False
            #             break
            #     if not isEqual:
            #         break
    ...
    def runMainLoop(self):
        ...
        for _ in range(0,self.numberOfGames,1):
            ...
            while self.gameOver == False:
                ...
                action = self.policyIterationPolicy[prsnt_state] 
