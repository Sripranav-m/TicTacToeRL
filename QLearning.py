...
class TicTacToe:
    ...
    def Q_learning(self):
        currState = 0
        # Start our trajectory with initial state 0 and go on...
        length=100000
        for i in range(length):
            if i%(length/100)==0:
                print(".",end="")
            board = self.decimalToBoard(self.statespace[currState])
            zeros_pos = self.getZeroesInBoard(board)
            random_action = random.choice(zeros_pos)
            # Take the action randomly according to their probabilities
            action_idx = self.naryToDecimal(np.array(random_action), self.boardDimension)
            # next state is chosen from all the possible next states from current states
            nxt_st = np.random.choice(range(self.stateSpaceLength), 1, p=self.PTF[currState][action_idx])[0]
            # Update our Q Table 
            # Qnew(st,at) = Q(st,at)+alpha(rewardatT + gamma* maxAQ(st+1,a)-Q(st,at)) 
            self.Q_table[currState][action_idx] = self.Q_table[currState][action_idx] + 0.1*(self.rewardFunction[currState][action_idx][nxt_st] + 0.1 * (np.max(self.Q_table[nxt_st])) - self.Q_table[currState][action_idx])
            currState = nxt_st

        # For each state in statespace, find the action that has maximum Q value and make our policy
        for i in range(len(self.statespace)):
            max_val = -sys.maxsize - 1
            best_action = []
            zeros_pos = self.getZeroesInBoard(self.decimalToBoard(self.statespace[i]))
            for j in zeros_pos:
                ...
                if self.Q_table[i][action_idx] > max_val:
                    ...
                    best_action = j.copy()
            self.qLearningPolicy.append(best_action)
    ...
    def runMainLoop(self):
        ...
        for _ in range(0,self.numberOfGames,1):
            ...
            while self.gameOver == False:
                ...
                action = self.qLearningPolicy[prsnt_state] 
