...
class TicTacToe:
    ...
    def defineVariables(self):
        ...
        self.gameOver = False
        # states * actions
        self.Q_table = np.zeros((self.stateSpaceLength, self.boardDimension**2))
        self.winner = 0
    ...
    # Take action in such a way that
    # (1-epsilon) for the greedy one
    # epsilon for the random state
    def epsilonGreedyAction(self,curr_state):
        actionArr = []
        epsilon = 0.1
        zeroes_pos = self.getZeroesInBoard(self.decimalToBoard(self.statespace[curr_state]))    
        max_val = -sys.maxsize-1
        best_action = []
        for i in zeroes_pos:
            action_idx = self.naryToDecimal(np.array(i), self.boardDimension)
            if self.Q_table[curr_state][action_idx] > max_val:
                max_val = self.Q_table[curr_state][action_idx]
                best_action = i.copy()
        actionArr.append(best_action)
        random_action = random.choice(zeroes_pos)
        actionArr.append(random_action)
        action = actionArr[np.random.choice(range(2), 1, p=[1-epsilon, epsilon])[0]]
        action_idx = self.naryToDecimal(np.array(action), self.boardDimension)
        return action_idx

    def SARSA(self):
        currState = 0
        currAction = self.epsilonGreedyAction(currState)
        length=100000
        for i in range(length):
            if i%(length/100)==0:
                print(".",end="")
            nextState = np.random.choice(range(len(self.statespace)), 1, p=self.PTF[currState][currAction])[0]
            # Take the action epsilon greedily
            # balance between exploration and exploitation
            nxt_action = self.epsilonGreedyAction(nextState)
            self.Q_table[currState][currAction] = self.Q_table[currState][currAction] + 0.1 * (self.rewardFunction[currState][currAction][nextState] + 0.3*self.Q_table[nextState][nxt_action] - self.Q_table[currState][currAction])
            currState = nextState
            currAction = nxt_action
            
        for i in range(len(self.statespace)):
            max_val = -sys.maxsize-1
            best_action = []
            zeroes_pos = self.getZeroesInBoard(self.decimalToBoard(self.statespace[i]))
            for j in zeroes_pos:
                action_idx = self.naryToDecimal(np.array(j), self.boardDimension)
                if self.Q_table[i][action_idx] > max_val:
                    max_val = self.Q_table[i][action_idx]
                    best_action = j.copy()
            self.sarsa_policy.append(best_action)
    ...
    def runMainLoop(self):
        ...
        for _ in range(0,self.numberOfGames,1):
            ...
            while self.gameOver == False:
                ...
                action = self.sarsa_policy[prsnt_state] 
